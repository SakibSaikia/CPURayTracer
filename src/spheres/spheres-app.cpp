...
void SpheresApp::InitScene()
{
	...
	for (int a = -11; a < 11; ++a)
	{
		for (int b = -11; b < 11; ++b)
		{
			...
			if (chooseMat < 0.8f)
			{
				m_scene.push_back(std::make_unique<Sphere>(center, 0.2f, std::make_unique<Dielectric>(
					XMCOLOR{
			...
			{
				m_scene.emplace_back(std::make_unique<Sphere>(center, 0.2f, std::make_unique<Transparent>(1.5f)));
			}
		}
	}
